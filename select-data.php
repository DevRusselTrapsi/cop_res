...
// catname variable from the ajax data in admin_dash
$catname = $_POST['cat_name'];

// check if the value from the select tag is not equal from all the other value will be checked
if($catname != 'all'){

	$cond = "'$catname'";

// order by asc a-z all the data from the database
$fetch_query = mysqli_query($conn, "SELECT * FROM tbl_resort WHERE verification = $cond ORDER BY resort_name ASC");
...
if($row > 0){
	...
	while ($res = mysqli_fetch_array($fetch_query)) {
        ...
        if($res['permit_url'] == 'no_permit'){
                ...
                <td>
                    <a href='./admin_verif.php?get=".$res['resort_id']."'><span class='status pending'>Pending</span>
                    </a>
                </td>
                <td>
                    <a href='./admin_verif.php?get=".$res['resort_id']."'></a>
